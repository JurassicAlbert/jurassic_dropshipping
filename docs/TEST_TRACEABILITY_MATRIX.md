...
- Playwright HTTP transport write-path stubs (`NEXT_PUBLIC_ADMIN_TRANSPORT=http` build): `admin_next/tests/e2e/http-write-route-stubs.spec.ts` — `page.route` latency + `429` on approval POST, incidents PATCH, return-policies POST (`npm run test:e2e:http-writes`; CI runs after mock E2E).
