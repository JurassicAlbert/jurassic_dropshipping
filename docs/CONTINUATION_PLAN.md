...
- [x] Add explicit route-stub latency/429 scenarios when HTTP write routes are introduced (`tests/e2e/http-write-route-stubs.spec.ts`, `npm run test:e2e:http-writes`; CI rebuilds with `NEXT_PUBLIC_ADMIN_TRANSPORT=http`).
