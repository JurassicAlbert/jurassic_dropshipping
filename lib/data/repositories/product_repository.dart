...
class ProductRepository {
  ...
  Future<List<Product>> getAll() async {
    final rows = await (_db.select(_db.products)..orderBy([(t) => OrderingTerm.desc(t.updatedAt)])).get();
    return rows.map(_rowToProduct).toList();
  }

  Future<Product?> getByLocalId(String localId) async {
    final row = await (_db.select(_db.products)..where((t) => t.localId.equals(localId))).getSingleOrNull();
  ...
  Future<Product?> getBySourceId(String sourcePlatformId, String sourceId) async {
    final row = await (_db.select(_db.products)
          ..where((t) => t.sourcePlatformId.equals(sourcePlatformId) & t.sourceId.equals(sourceId)))
        .getSingleOrNull();
    return row != null ? _rowToProduct(row) : null;
  }
  ...
  Future<void> deleteByLocalId(String localId) async {
    await (_db.delete(_db.products)..where((t) => t.localId.equals(localId))).go();
  }
}
